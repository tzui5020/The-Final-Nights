/obj/item/reagent_containers/drug/bweedpack
	...
	list_reagents = list(/datum/reagent/drug/cannabis = 20, /datum/reagent/toxin/lipolicide = 20) //some of the source chemicals are lost in the process
...
/obj/item/reagent_containers/drug/bweedpack/attack(mob/living/M, mob/living/user)
	. = ..()
	if(!iskindred(M))
		if(!vitae)
			return
	...
	if(do_mob(user, src, 3 SECONDS))
		var/obj/item/reagent_containers/drug/bweedpack/H = new(src) //setting up the drugged bag in question (and its contents) as a variable to be called later
		feeding = FALSE
		empty = TRUE
		icon_state = "blood0"
