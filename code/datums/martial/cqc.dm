...
/datum/martial_art/cqc/teach(mob/living/new_holder)
	. = ..()
...
/datum/martial_art/cqc/proc/check_block(mob/living/cqc_user, atom/movable/hitby, damage, attack_text, attack_type, ...)
	...
	if(!can_use(cqc_user) || !cqc_user.throw_mode || cqc_user.incapacitated())
		return NONE
	...
	if(attacker.grab_state == GRAB_KILL \
		...
		if(!isnull(head))
			...
			if(!HAS_TRAIT(defender, TRAIT_NODEATH))
				defender.death()
				log_combat("has had [defender.p_their()] neck snapped by [attacker].")
			return MARTIAL_ATTACK_SUCCESS
